...
def _overview_assets() -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    paths = simulate_gbm(n_paths=40, n_steps=64, antithetic=False, seed=3)
    data = make_european_dataset(n_samples=2_000, seed=7)
    return paths, data
...
Pick a page above — every chart is computed live from this repo's library.
    ...
    paths, data = _overview_assets()
    row1_l, row1_r = st.columns(2)
    with row1_l:
        st.markdown("**The underlying** — 40 GBM paths, colored by where "
                    "they end")
        fin = paths[:, -1]
        ...
        for i in range(paths.shape[0]):
            fig.add_trace(go.Scatter(y=paths[i].tolist(), mode="lines",
    ...
    with row1_r:
        st.markdown("**The book** — short one call, collect 9.83, owe the "
                    "payoff")
        ...
        fig.add_trace(go.Scatter(
            ...
            hoverinfo="skip", showlegend=False))
        fig.add_trace(go.Scatter(x=s_t.tolist(), y=profit.tolist(),
                                 mode="lines", line=dict(color="#333333"),
                                 name="profit at expiry"))
        fig.add_trace(go.Scatter(x=s_t.tolist(), y=payoff.tolist(),
                                 mode="lines",
                                 line=dict(color="#636EFA", dash="dash"),
                                 name="payoff owed"))
        fig.add_vline(x=100.0, line_dash="dot", line_color="gray",
    ...
    with row2_l:
        st.markdown("**What the pricer computes** — option value over spot "
                    "and volatility")
        ...
        fig = go.Figure(go.Heatmap(x=vols.tolist(), y=spots.tolist(),
        ...
        st.plotly_chart(fig, width="stretch")
    with row2_r:
        st.markdown("**What the models learn from** — Sobol coverage of the "
                    "parameter space")
        xs = data["x_train"][:1500]
        fig = go.Figure(go.Scatter(
            x=xs[:, 0].tolist(), y=xs[:, 3].tolist(), mode="markers",
            marker=dict(size=4, color=(xs[:, 0] / xs[:, 1]).tolist(),
                        colorscale="Viridis", showscale=True,
                        colorbar=dict(title="s0/K", thickness=10))))
        fig.update_layout(height=300, template=TEMPLATE,
                          margin=dict(l=10, r=10, t=10, b=10))
